...
const FCFS = () => {
  ...
  const simulateFCFS = () => {
    ...
    const sorted = [...processes].sort((a, b) => parseInt(a.arrival) - parseInt(b.arrival));
    const tempTimeline = [];
    let currentTime = 0;

    sorted.forEach(proc => {
      const arrival = parseInt(proc.arrival);
      const burst = parseInt(proc.burst);

      // Idle if arrival is later than current time
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-6 md:p-12">
      <h1 className="text-3xl font-bold mb-6 text-indigo-700">FCFS Scheduling Simulator</h1>
            ...
            <div className="flex gap-2">
              {timeline.map((slot, idx) => (
                <div
                  ...
                  style={{ width: `${slot.duration * 40}px` }} // scale width to duration
                >
